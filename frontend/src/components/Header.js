...
const Header = ({ keyword: propKeyword, setKeyword: propSetKeyword, onSearch, location: propLocation, setLocation: propSetLocation }) => {
  ...
  // LẤY THÔNG TIN USER VÀ ĐẾM GIỎ HÀNG/THÔNG BÁO
  useEffect(() => {
  ...
  return (
    <header className="bg-white border-bottom shadow-sm sticky-top" style={{ zIndex: 1040, top: 0 }}>
      
      {/* HEADER CHÍNH: Logo + Tìm kiếm + User */}
      <div className="container py-2 d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
        ...
        <div className="d-flex flex-grow-1 align-items-center gap-2 w-100 w-md-auto mt-2 mt-md-0 position-relative">
          ...
          {/* MODAL ĐỊA ĐIỂM */}
          {showLocationModal && (
              <div className="position-absolute bg-white shadow-lg border rounded-3 p-3 z-index-dropdown" style={{ top: '50px', left: '0', width: '300px', zIndex: 1050 }}>
          ...
          {user ? (
            <div className="d-flex align-items-center gap-3">
                ...
                <div className="position-relative">
                    ...
                    />
                    {showProfileMenu && (
                        <div className="position-absolute bg-white shadow-lg rounded-4 p-3 z-index-dropdown" style={{ right: 0, top: '50px', width: '280px', border: '1px solid #eee', zIndex: 1050 }}>
