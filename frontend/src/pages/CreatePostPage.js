        ...
        <form onSubmit={handleSubmit} className="bg-white p-4 rounded-4 shadow-sm mx-auto" style={{maxWidth: '700px'}}>
          ...
          <div className="mb-3">
            ...
            <div className="d-flex gap-2 mt-3 flex-wrap">
                {previewImages.map((src, i) => <img key={i} src={src} className="rounded-3 border" style={{width: '80px', height: '80px', objectFit: 'cover'}} />)}
            </div>
          </div>
          <div className="mb-3">
            <label className="form-label fw-bold small text-muted">Tiêu đề</label>
            <input type="text" className="form-control bg-light px-3" required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
          </div>
